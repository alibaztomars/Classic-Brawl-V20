...
from Utils.acc_loader import acc_loader
...
from Packets.Messages.Server.WarMessage import WarMessage
...
class LoginMessage(BSMessageReader):
    ...
    def process(self):
        ...
        if self.fingerprint_sha != self.player.patch_sha and self.player.patch:
            ...
            player_data = DataBase.loadAccount(self) # load account
            acc_loader.load(self, player_data)
            ...
            LoginOkMessage(self.client, self.player).send()
            #WarMessage(self.clent, self.player).send()
            OwnHomeDataMessage(self.client, self.player).send()
            #MyAllianceMessage(self.client, self.player, self.player.club_low_id).send()
